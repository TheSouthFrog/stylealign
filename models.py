...
def dec_up(c, init = False, dropout_p = 0.5, n_scales = 1, n_residual_blocks = 2, activation = "elu", n_filters = 64, max_filters = 128):
...
def dec_down(gs, zs_posterior, training, init = False, dropout_p = 0.5, n_scales = 1, n_residual_blocks = 2, activation = "elu", n_latent_scales = 2):
...
def enc_up(x, c, init = False, dropout_p = 0.5, n_scales = 1, n_residual_blocks = 2, activation = "elu", n_filters = 64, max_filters = 128):
    with model_arg_scope(
            init = init, dropout_p = dropout_p, activation = activation):
        hs = []
...
def enc_down(gs, init = False, dropout_p = 0.5, n_scales = 1, n_residual_blocks = 2, activation = "elu", n_latent_scales = 2):
    assert n_residual_blocks % 2 == 0
    gs = list(gs)
    with model_arg_scope(
            init = init, dropout_p = dropout_p, activation = activation):
...
def latent_sample_ag(p,c):
    mean = p
    stddev = 1.0
    eps = tf.random_normal(mean.shape, mean = 0.0, stddev = 1.0)
    return mean + stddev * eps
    
def latent_sample_gmm(p,c):
    mean = p
    stddev = 1.0
    eps = tf.random_normal(mean.shape, mean = 0.0, stddev = 1.0)
    return mean + stddev * eps

def latent_kl_ag(q,p,w):
    mean1 = q
    mean2 = p
